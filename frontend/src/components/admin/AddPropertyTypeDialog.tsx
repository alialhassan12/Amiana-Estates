...
import { addPropertyTypeSchema, type AddPropertyTypeFormData } from "../../schemas/addPropertyTypeSchema";
...
const emptyFormValues: AddPropertyTypeFormData = {
    title: "",
    description: "",
    area: 0,
    imageFile:undefined,
...
const AddPropertyTypeDialog = ({ open, setOpen, displayOrder }: AddPropertyTypeDialogProps) => {
    ...
    const {
        ...
        setValue,
        reset,
    } = useForm<AddPropertyTypeFormData>({
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setValue("previewImage", URL.createObjectURL(file), { shouldDirty: true });
    };
    ...
    const onSubmit = async (data: AddPropertyTypeFormData) => {
        try {
            const formData = new FormData();
            formData.append("title", data.title);
            formData.append("description", data.description);
            formData.append("area", data.area.toString());
            formData.append("area_unit", "m²");
            ...
            <DialogContent className="sm:max-w-2xl h-[min(44rem,calc(100vh-2rem))] max-h-[calc(100vh-2rem)] !grid !grid-rows-[auto_minmax(0,1fr)] !gap-0 !p-0 overflow-hidden">
                <DialogHeader className="shrink-0 border-b border-[#ECE9E5] px-4 py-4 pr-12">
                    ...
                    <DialogDescription className="body-text text-xs leading-relaxed text-neutral-500">
                        Create a residence type for the website catalogue.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="grid min-h-0 grid-rows-[minmax(0,1fr)_auto]">
                    <div className="min-h-0 overflow-y-auto custom-scrollbar">
                        <div className="p-4 sm:p-6 space-y-6">
                            ...
                            <div>
                                <label htmlFor="new-property-type-description" className="block uppercase body-text text-[11px] font-semibold tracking-wider text-neutral-700 mb-1.5">
                                    Description
                                </label>
                                <textarea
                                ...
                                <div className="flex items-start justify-between gap-3 mt-1.5">
                                    ...
                                    <p className="body-text text-[10px] text-neutral-400 text-right">This text is displayed on the residence card.</p>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-[#ECE9E5] pt-5">
                                <div className="mb-3">
                                    ...
                                    <p className="body-text text-[11px] text-neutral-500 mt-1">Add the image that represents this residence type.</p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-[10rem_1fr] gap-4 items-stretch">
                                    <div className="aspect-[4/3] sm:aspect-auto min-h-32 rounded-md overflow-hidden bg-[#ECE9E5] border border-[#ECE9E5]">
                                        {previewImage ? (
                                            <img src={previewImage} alt="New property type preview" className="w-full h-full object-cover" />
                                        ) : (
                                            <div className="w-full h-full min-h-32 flex flex-col items-center justify-center gap-2 text-neutral-400">
                                                <ImageIcon className="h-5 w-5" />
                                                <span className="body-text text-[10px] uppercase tracking-wider">No image selected</span>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div className="border-t border-[#ECE9E5] pt-5">
                                ...
                                <button
                                    type="button"
                                    aria-pressed={isPenthouse}
                                    onClick={() => setValue("is_penthouse", !isPenthouse, { shouldDirty: true, shouldValidate: true })}
...
export default AddPropertyTypeDialog;
